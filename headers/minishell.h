...
void				update_env(t_env **env, const char *name,
						const char *value);
int					get_nbr_arg(char **arg);
int					cd_zero_arg(char *tmp, char *pwd_now, char *cur,
						t_env *env);
int					cd_whith_1p(char *pwd_now, char *cur, t_env *env);
int					cd_whith_2p(char *pwd_now, char *cur, t_env *env);
int					cd_with_arg(char **arg, char *pwd_now, char *cur,
						t_env *env);
...
void				close_other_fil(int pipe[][2], int nbr_pipe, int fd1,
						int fd2);
...
int					create_pipe_if_needed(t_cmd *current, int pipefd[2],
						int prev_pipe);
void				setup_child_redirections(t_cmd *current, int prev_pipe,
						int pipefd[2]);
void				execute_child_command(t_cmd *current, t_env **env,
						int status);
...
int					execute_with_redirection(t_cmd *current, t_env **env,
						int status);
...
void				add_back_env(t_env **env, char *name, char *value,
						int index);
